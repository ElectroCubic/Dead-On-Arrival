...
@onready var rollover_sfx: AudioStreamPlayer = $RolloverSfx
...
var current_music_vol: float = 1.0
...
func fade_out_music() -> void:
	...
	tween.tween_method(control_vol_music,current_music_vol,0.0,music_fade_out_time)
	await tween.finished

func fade_in_music() -> void:
	var tween: Tween = get_tree().create_tween()
	tween.tween_method(control_vol_music,0.0,current_music_vol,music_fade_in_time)
	await tween.finished
